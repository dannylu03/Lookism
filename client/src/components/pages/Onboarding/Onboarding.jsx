...
const Onboarding = ({ user }) => {
  ...
  const updatingUser = () => {
    axios
      .put(`http://localhost:8000/users/update/${user._id}`, {
        username: user.name,
        tags: userStyles,
      })
      .then((res) => {
  ...
  return (
    <div className="bg-cultured w-screen h-screen flex flex-col justify-center items-center">
      ...
      <div className="bg-timberwolf w-4/5 h-5/6 rounded-xl relative">
        <h1 className="text-cafe-noir p-3 font-serif">
      ...
      <button
        ...
        className="w-1/4 bg-camel text-white font-serif text-xl rounded-3xl p-2 mt-2"
      >
        Continue
      </button>
    </div>
